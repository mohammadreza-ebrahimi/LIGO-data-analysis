...
# ## <span style="color:red">  BINARY BLACK HOLE SIGNALS IN LIGO OPEN DATA </span>  
# 
# This is a tutorial script for one who wants to be familiar with gravitational waves data analysis
...
# **Note**: LIGO Hanford (H1), LIGO Livingston (L1)
...
print("Reading in parameters for event %s" % event["name"])
print(event)
...
print('time_H1: len: %10f ' % len(time_H1) , 'min: %10f' % time_H1.min() ,
      'mean: %10f' % time_H1.mean(), 'max: %10f' % time_H1.max())

print('strain_H1: len: ', len(strain_H1) , 'min: ' , strain_H1.min() ,
      'mean: ', strain_H1.mean(), 'max: ', strain_H1.max())  

'''
Here if we specify the type like "f" or "d" ,
it wont show the exact min, max and mean value
'''

print('strain_L1: len: ', len(strain_L1) , 'min: ' , strain_L1.min() ,
      'mean: ', strain_L1.mean(), 'max: ' , strain_L1.max())
...
for i in np.arange(1, 5, 0.5):
    detat = i
#deltat = 2
    indxt = np.where((time >= tevent-i) & (time < tevent+i))
    plt.figure()
